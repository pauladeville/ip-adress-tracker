 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function ve(e,t={}){let n=e;if("object"!=typeof t){t={name:t}}const r=Object.assign({name:ue,automaticDataCollectionEnabled:!1},t),i=r.name;if("string"!=typeof i||!i)throw be.create("bad-app-name",{appName:String(i)});if(n||(n=I()),!n)throw be.create("no-options");const a=pe.get(i);if(a){if(O(n,a.options)&&O(r,a.config))return a;throw be.create("duplicate-app",{appName:i})}const s=new R(i);for(const e of fe.values())s.addComponent(e);const o=new ye(n,r,s);return pe.set(i,o),o}function Ee(e,t,n){var r;let i=null!==(r=de[e])&&void 0!==r?r:e;n&&(i+=`-${n}`);const a=i.match(/\s|\//),s=t.match(/\s|\//);if(a||s){const e=[`Unable to register library "${i}" with version "${t}":`];return a&&e.push(`library name "${i}" contains illegal characters (whitespace or "/")`),a&&s&&e.push("and"),s&&e.push(`version name "${t}" contains illegal characters (whitespace or "/")`),void le.warn(e.join(" "))}ge(new N(`${i}-version`,(()=>({library:i,version:t})),"VERSION"))}
/**
 * @license
 * Copyright 2021 Google LLC
 *
